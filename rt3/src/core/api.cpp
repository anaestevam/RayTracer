...
void render(const std::shared_ptr<Scene> &s) {
  ...
  auto lower_left_corner =
      origin - horizontal / 2 - vertical / 2 - Vector3f(0, 0, focal_length);
  ...
  for (size_t y = 0; y < h; ++y) {
    for (size_t x = 0; x < w; ++x) {
      ...
      Point2f pixel_coords{static_cast<float>(x) / static_cast<float>(w),
                           static_cast<float>(y) / static_cast<float>(h)};
      ColorXYZ color{0, 0, 0};
      // Ray r(origin, Vector3f{1,1,1} * (lower_left_corner + horizontal*u +
      // vertical*v - origin));

      if (s->backgroundColor->mapping_type == Background::mapping_t::screen)
        color = s->backgroundColor->sampleXYZ(pixel_coords);
      else if (s->backgroundColor->mapping_type ==
               Background::mapping_t::spherical)
        color = s->backgroundColor->sampleXYZ(pixel_coords);
...
BackgroundColor *API::make_background(const std::string &type,
                                      const ParamSet &ps) {
...
API::make_primitives(const std::vector<ParamSet> &object_params) {
  ...
  for (const auto &ps : object_params) {
    ...
    if (primitive != nullptr) {
      prim_list.emplace_back(std::shared_ptr<Primitive>(primitive));
    }
  }
  return prim_list;
}
...
void API::world_end() {
  ...
  auto *cam = make_camera(render_opt->camera_type, render_opt->camera_ps, render_opt->lookat_ps, the_film);
  std::shared_ptr<Scene> scene = std::make_shared<Scene>(
      cam, std::move(the_background), make_primitives(render_opt->object_ps));
  // std::shared_ptr<const camera> camera(setup_camera());
  //  Run only if we got film and background.
  if ((the_film != nullptr) and the_background) {
